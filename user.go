...
func addUser(user *User) (bool, error) {
	...
	// check if the user has alrady existed
...
func handlerLogin(w http.ResponseWriter, r *http.Request) {
	...
	// create a token using NewWithClaims
	// jwt.SigningMethodHS256: method of encoding
	// exp: set expiration time (when you have to resign)
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"username": user.Username,
		"exp":      time.Now().Add(time.Hour * 24).Unix(),
	})

	// Sign using secret key and get the complete token
...
// ResponseWriter is an interface. Therefore, cannot add * to it.
